    ...
    return(
        <ScrollView style={styles.rootElement}>
            ...
            <View style={{display: "flex",flexDirection: "row",justifyContent: "space-between",alignItems: "center",borderBottomColor: "rgba(0,0,0,0.5)",borderBottomWidth: "1px"}}>
                ...
                <Text style={{fontWeight: "bold",textTransform: "uppercase",fontSize: "16px"}}>gift's rejected by you</Text>
                <View />
            </View>
            {/* TABLE HEADER */}
            <View style={styles.tableHeader}>
                ...
                <Text style={styles.headerCell}>requested by</Text>
                <Text style={styles.headerCell}>view form</Text>
            </View>
            {/* TABLE BODY */}
            <View style={styles.tableBody}>
                {(giftRejectedList instanceof Array && giftRejectedList.length > 0) ?
                    giftRejectedList.map((rejection) => {
                        return(
                            <View key={rejection.approvalID} style={styles.tableBodyRow}>
                                <Text style={styles.bodyCell}>#GH-{rejection.giftID}</Text>
                                <Text style={styles.bodyCell}>{rejection.rejectedReason}</Text>
                                <Text style={styles.bodyCell}>{rejection.requestedBy}</Text>
                                <FontAwesome5 onPress={() => navigation.navigate("gift-and-hospitality-form",{giftID: rejection.giftID,canEdit: false})} style={{...styles.bodyCell}} name="readme" size={30} color="blue" />
                            </View>
                        )
